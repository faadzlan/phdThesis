...
\begin{abstract}
This work studies an energy harvesting mechanism from fluid flows using an elastically supported circular cylinder and a downstream strip plate in cruciform formation. The Reynolds number range is $1.1 \times 10^{3} \leq \text{Re} \leq 14.6 \times 10^{3}$ and Scruton number $\text{Sc} = 9.94$. This range of Re and Sc is similar to other circular cylinder oscillator studies.
The primary mode of investigation is numerical. However, experiments are done to validate the numerical setup at the beginning stages of the study. The governing equations are solved using OpenFOAM, a free and open-source C++ library. The numerical setup is validated by experiments done in a recirculating open flow channel. The validation process consists of comparing the vibration amplitude results between the numerical and experimental study of a $90 \si{\degree}$ circular cylinder - strip plate cruciform.
The three-dimensional velocity field is post-processed to visualise vorticity and vortex shedding frequency. The three-dimensional pressure field is post-processed into lift acting on the cylinder. The cylinder displacement is post-processed into cylinder vibration amplitude and frequency. Then, the signals are processed using the Hilbert-Huang transform to uncover the major components of their signals, instantaneous phase difference between cylinder displacement and lift, and their instantaneous frequencies. This is repeated for cruciform angles $ 67.5 \si{\degree} $, $ 45 \si{\degree} $, $ 22.5 \si{\degree} $, and $ 0 \si{\degree} $.
